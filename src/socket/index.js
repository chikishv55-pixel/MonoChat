...
module.exports = function(io, onlineUsers) {
    ...
    io.use(async (socket, next) => {
        ...
        try {
            ...
            const user = await dbGet(`SELECT username, is_admin, is_banned, display_name, avatar, bio, birth_date, music_status, fcm_token, is_premium, profile_card_bg, profile_effect FROM users WHERE username = ?`, [decoded.username]);
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('search users', async (query, callback) => {
            ...
            try {
                ...
                const me = socket.user;
                const userRows = await dbAll(`SELECT username, display_name, avatar, bio, birth_date, music_status, is_premium, profile_card_bg, profile_effect FROM users WHERE LOWER(username) = LOWER(?) LIMIT 1`, [query.trim()]);
